...
web:
  ...
  admins:
    - username: "admin"
      password_hash: "<admin-password>"  # 启动时自动 bcrypt 化
...
当前生产环境部署在 **<deploy-host>** (<hostname>)：
...
SSHPASS='<password>' sshpass -e rsync -avz \
  --exclude='.git' --exclude='bin' --exclude='*.db' --exclude='qdrant' \
  /tmp/agent-memory/ <deploy-host>:<src-dir>/
...
# 构建
go build -o <deploy-dir>/agent-memory-server ./cmd/server/

# 3. 重启服务
kill $(pgrep -f agent-memory-server)
cd <deploy-dir> && nohup ./agent-memory-server > /tmp/am.log 2>&1 &
```
...
```bash
make deploy DEPLOY_HOST=<deploy-host> DEPLOY_DIR=<deploy-dir>
```
...
```bash
# 检查服务状态
curl http://<deploy-host>:8101/api/v1/health
...
所有 Agent 共享同一个 API Key `<your-api-key>`（开发环境），通过 `X-User-Id` header 隔离记忆空间。生产环境应使用独立 API Key。
...
# API Key 认证测试
curl -H "X-API-Key: <your-api-key>" http://localhost:8101/api/v1/memories

# JWT 登录测试
TOKEN=$(curl -s -X POST http://localhost:8101/api/v1/auth/login \
  -H "Content-Type: application/json" \
  -d '{"username":"admin","password":"<admin-password>"}' | python3 -c "import sys,json;print(json.load(sys.stdin)['token'])")
...
```bash
curl -H "X-API-Key: <your-api-key>" http://localhost:8101/api/v1/memories/report
```
...
```bash
# 热备份（SQLite 支持读时复制）
cp <deploy-dir>/data/memories.db /backup/memories-$(date +%Y%m%d).db
```
